...
static inline void mi_bchunk_clear_once_set(mi_subproc_t* subproc, mi_bchunk_t* chunk, size_t cidx) {
  mi_assert_internal(cidx < MI_BCHUNK_BITS);
  const size_t i = cidx / MI_BFIELD_BITS;
  const size_t idx = cidx % MI_BFIELD_BITS;
...
// The loads are acquire: `mi_heap_delete` walks its `arena_pages->pages` with this and then frees the heap and
// the bitmap itself, while a concurrent `mi_free` that frees a page of the heap reads both and then clears the
// page's bit (release) as its last access (`mi_arenas_page_free_prim`). Seeing that bit clear here is what
