...
static mi_option_desc_t mi_options[_mi_option_last] =
{
  ...
  { 240, MI_OPTION_UNINIT, MI_OPTION(os_tag) },                   // macOS VM tag: 240 = VM_MEMORY_APPLICATION_SPECIFIC_1 (100 collided with IOAccelerator)
  ...
  { 100, MI_OPTION_UNINIT, MI_OPTION(purge_holes_min_interval) },  // min msecs between idle sweeps of one thread's heaps (=100); and the free blocks of a large page stay for this long after the last allocation from it (that part holds for every sweep, also one that nothing rate-limits)
...
// fork: informational output (heap snapshot / profile file names and the like) that is not gated on the verbose option
