...
static bool mi_theap_page_collect(mi_theap_t* theap, mi_page_queue_t* pq, mi_page_t* page, void* arg_collect, void* arg2 ) {
  ...
  _mi_page_free_collect_no_unpurge(page, collect >= MI_FORCE);  // update used count
  _mi_page_update_stats_for(page,theap);   // (on `theap`, which is not the calling thread's when the scavenger collects for a parked thread)
...
static void mi_purge_holes_of(mi_tld_t* tld) mi_attr_noexcept {
  ...
  _mi_page_purge_holes_sweep_end(tld);
}
  ...
  for (;;) {
    ...
    mi_lock(&subproc->tlds_lock) {
      ...
      for (mi_tld_t* tld = subproc->tlds; tld != NULL; tld = tld->subproc_next) {
        // Done for this park? But for what it left under `purge_holes_large_floor`: if the thread is still parked
        // `purge_holes_large_floor_epochs` intervals later, that goes (`_mi_page_purge_holes_floor_due_in`), and we say when so
        // that the scavenger is back by then: once, since that sweep leaves nothing. A thread that stays parked is not to
        // hold the floor against the ones that run, and a process that goes idle gives it back.
        // (Its fields are read once it is claimed: its owner writes them when it sweeps itself.)
        ...
        if (mi_atomic_cas_strong_acq_rel(&tld->park_state, &expected, MI_PARK_SWEEPING)) {
          if (done) {
            ...
            if (floor_due != 0 || mi_atomic_load_relaxed(&tld->park_reclaim) != 0) {
              if (floor_due > 0 && (due_in == 0 || floor_due < due_in)) { due_in = floor_due; }
...
bool _mi_theap_area_visit_blocks(const mi_heap_area_t* area, mi_page_t* page, mi_block_visit_fun* visitor, void* arg) {
  ...
  mi_assert_internal(page->capacity == (free_count + purged_count + mi_page_used(page)));
