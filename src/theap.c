...
static bool mi_theap_page_collect(mi_theap_t* theap, mi_page_queue_t* pq, mi_page_t* page, void* arg_collect, void* arg2 ) {
  ...
  if (mi_page_thread_id(page) == MI_THREADID_FROZEN) return true;   // snapshot page: never freed, abandoned or rewritten
...
static bool mi_theap_page_purge_holes(mi_theap_t* theap, mi_page_queue_t* pq, mi_page_t* page, void* arg1, void* arg2) {
  ...
  if (theap->tld != NULL && mi_atomic_load_relaxed(&theap->tld->park_reclaim) != 0) return false;
  if (mi_page_thread_id(page) == MI_THREADID_FROZEN) return true;   // snapshot page: its bytes are the snapshot file's
...
// Mark a theap as an immutable snapshot: idle work, hole purging and collection skip it from now on.
void mi_theap_freeze(mi_theap_t* theap) mi_attr_noexcept {
  if (theap == NULL) return;
  theap->frozen = true;
}

// Snapshot restore: the thread state in the snapshot (the main thread's static tld included) records the
