...
#include <pthread.h> // pthread_atfork (fork handlers, subproc.c)
#endif

static void mi_tld_register(mi_tld_t* tld);   // fork: tld registry for the scavenger (defined with mi_tld_free)
...
static const mi_page_t mi_page_empty = {
  ...
  MI_PAGE_SWEPT_NONE      // swept_state (fork)
};
...
static mi_decl_cache_align mi_tld_t mi_tld_detached = {
  ...
  MI_MEMID_STATIC,        // memid
  MI_ATOMIC_VAR_INIT(0),  // park_state = running (fork)
  ...
  0,                      // holes_floor_kept
  0, 0, NULL              // holes_sweep_ticks / holes_floor_due / holes_floor_list
};
...
// fork: register a tld with its subproc so the scavenger can find it when it parks (`mi_on_thread_idle_start`).
// Idempotent: a tld can come through allocation more than once if the thread is re-initialized after `_mi_thread_done`.
...
// fork: unlink before the tld memory goes away. The tld cannot be mid-sweep here: only a PARKED tld is ever claimed,
// and a thread running its own teardown is RUNNING by definition.
