...
static bool _mi_stats_get(const mi_stats_t* stats_in, mi_stats_t* stats_out) mi_attr_noexcept {
...
bool mi_subproc_stats_get_exclusive(mi_subproc_id_t subproc_id, mi_stats_t* stats) mi_attr_noexcept {
  ...
  return _mi_stats_get(&subproc->stats, stats);
}
...
bool mi_subproc_stats_get(mi_subproc_id_t subproc_id, mi_stats_t* stats) mi_attr_noexcept {
  mi_subproc_t* subproc = _mi_subproc_from_id(subproc_id);
  if (subproc == NULL || stats == NULL || stats->size != sizeof(mi_stats_t) || stats->version != MI_STAT_VERSION) return false;
...
static bool mi_dump_block_visit(const mi_heap_t* heap, const mi_heap_area_t* area, void* block, size_t block_size, void* arg) {
  MI_UNUSED(heap);
  mi_dump_ctx_t* ctx = (mi_dump_ctx_t*)arg;
  char tmp[128];
  if (block == NULL) {
    if (ctx->in_block_pass) return true;
    if (!ctx->first_page) { mi_json_buf_print(&ctx->hbuf, ",\n"); }
    ...
    mi_json_buf_print(&ctx->hbuf, tmp);
  }
  else if (ctx->in_block_pass) {
    if (!ctx->first_block) { mi_json_buf_print(&ctx->hbuf, ","); }
...
static bool mi_dump_heap_visit(mi_heap_t* heap, void* arg) {
  mi_dump_ctx_t* ctx = (mi_dump_ctx_t*)arg;
  char tmp[64];
  if (!ctx->first_heap) { mi_json_buf_print(&ctx->hbuf, ",\n"); }
