...
mi_decl_export bool mi_heap_image_hints_enabled(void) mi_attr_noexcept;   // deterministic placement is on for this process (image-capable executable, or MIMALLOC_DETERMINISTIC_HINT=1)
...
mi_decl_export void mi_theap_adopt_current_thread(mi_theap_t* theap) mi_attr_noexcept;  // image restore: this theap's thread state now belongs to the calling thread
