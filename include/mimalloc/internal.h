...
#define MI_HOLES_ASKED_AT_FREE     (0)
#define MI_HOLES_ASKED_BY_COLLECT  (1)
#define MI_HOLES_ASKED_IN_SWEEP    (2)
bool          _mi_page_purge_holes_free_page_stays(mi_page_t* page, mi_tld_t* tld, int asked);   // a large page with no block in use: is it left as it is for now?
#define MI_HOLES_FLOOR_LIST_MAX   (64)
typedef struct mi_holes_floor_item_s {
  mi_page_t* page;
  uint32_t   age;       // epochs since the page was last allocated from: smaller is more recent
  size_t     bytes;
