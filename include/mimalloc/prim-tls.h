...
#elif defined(__APPLE__) && !defined(__POWERPC__)   // fixed slot on macOS arm64/x64 (fastest); see the slot choice below
#define MI_TLS_MODEL_FIXED        1
...
static inline void* mi_prim_tls_slot(size_t slot) {
    ...
    // states that this is by design (and keeps TSAN quiet about it).
