...
- `MIMALLOC_ALLOW_THP=[0,1,2]`: When set to `0`, mimalloc opts the memory it maps itself out of transparent huge pages (THP)
   (per mapping, so this does not affect programs spawned by the process, nor memory handed to mimalloc via
   `mi_manage_os_memory`), and otherwise THP is used whenever it is [enabled in the OS][linux-thp] (`[always]` or `[madvise]`). If set to 2 (default on Linux),
