...
/* Release heaps on many threads at once while other threads free into them.
   ...
   the heap that is about to go (`theap.c:mi_purge_holes_of`, `_mi_arenas_purge_abandoned_holes`).
   Half of those heaps are deleted by the parked thread itself right after `mi_on_thread_idle_end`
   (racing the tail of the sweep), the other half by a freer thread *while the owner is still parked*
   (a heap created on one thread and released on another while the first sits idle in the kernel),
   so `mi_heap_delete`'s detach has to get past the scavenger holding that thread's theaps.
